...
LogP Viterbi(MyMap &map,Ngram &lm,VocabString *words,unsigned int wordcnt,Vocab &voc,Vocab &vocz,Vocab &vocb);
...
{
    /*
    File file("./segmenttexts/1.txt","r");
    char *content;
...
LogP Viterbi(MyMap &map,Ngram &lm,VocabString *words,unsigned int wordcnt,Vocab &voc,Vocab &vocz,Vocab &vocb)
{
    ...
    for(int t=1;t<wordcnt;t++)
    {
        VocabIndex wbig5idx;
        Prob p;
        MyMapIter iter(map,getIndex(vocz,words[t]));
